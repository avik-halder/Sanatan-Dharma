    ...
    <>
      ...
      {isHomePage && (
        <motion.div
          ...
          animate={controls}
        >
          <img src={logo3} alt="Logo" className="h-7 w-7 md:w-9 md:h-9 lg:h-11 lg:w-11 rounded-full" />
        </motion.div>
      )}

      {/* Navbar Structure */}
      <div className={`fixed z-50 mx-auto lg:left-8 lg:right-8 left-0 right-0 top-2 w-[95%] lg:max-w-[84%] ${isScrolled || isMenuOpen || !isHomePage ? 'bg-[#fff0d5e0] shadow-md border border-gray-300 rounded-2xl' : ''}`}>
        <nav className="px-4 py-2 sm:px-6 sm:py-3 flex justify-between items-center">
          ...
          <div className="flex items-center space-x-3">
            {(!isHomePage || isMenuOpen) && (
              <>
                <a href='/'><img src={logo3} alt="Logo" className="h-9 w-9 rounded-full" /></a>
                <a href='/' className="text-[25px] sm:text-4xl font-extrabold font-underdog text-[#CE6400]">
                  Sanatan Dharma
                </a>
              </>
            )}
            ...
            {(isHomePage && isScrolled && !isMenuOpen) && (
              <>
                <div id="logo-container" ref={logoContainerRef} className="h-9 w-9 rounded-full" />
        ...
        <div className="lg:hidden fixed inset-0 bg-orange-50 z-40 pt-20">
          <div className="h-full flex flex-col">
            <div className="flex-grow px-6 overflow-y-auto">
              {menuItems.map(item => (
                <a key={item.label} href={item.href} className="flex items-center justify-center space-x-3 py-5 text-amber-700 border-b-2 border-dashed border-orange-300">
                  <span className="text-2xl font-medium">{item.label}</span>
                </a>
              ))}
            </div>
            <div className="px-6 py-8 border-t border-gray-200">
              <span className="flex justify-center mb-1 text-gray-600">Want to connect with us?</span>
              <a href="/contact" className="block w-full rounded-md bg-amber-600 px-4 py-3 text-center text-2xl font-semibold text-white">Contact</a>
            </div>
          </div>
        </div>
      )}
